...
public:
    ...
    TreeNode * buildTree(vector<int> & preorder, int prestart, int preend, vector<int> & inorder, int instart, int inend, map<int,int> & mpp)
    {
        ...
        root->left = buildTree(preorder,prestart+1,prestart+numsleft,inorder,instart,inind-1,mpp);
        root->right = buildTree(preorder,prestart+numsleft+1,preend,inorder,inind+1,inend,mpp);
